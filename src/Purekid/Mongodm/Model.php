...
abstract class Model
{
    ...
    public function mutate($updateQuery, $options = array())
    {
        ...
        try {
            ...
            //update cleanData
            foreach($updateQuery as $key => $val){
              switch($key){
                case '$push':
                  foreach($val as $k1 => $v1){
                    if(key($v1) == '$each'){
                      ...
                      $this->cleanData[$k1][] = $val[$k1];
                    }
                  }
                  continue;
                case '$inc':
                  $this->cleanData[key($val)] +=  $val[key($val)];
              }
            }
    ...
    * @return Model
    */
    public function sortOne($criteria, $sort=array(), $fields = array()){
        ...
        $lines = $conn->find($criteria, $fields)->sort($sort)->limit(1);

        $models = \Purekid\Mongodm\Hydrator::hydrate($collectionName, $lines, "collection" , true);
